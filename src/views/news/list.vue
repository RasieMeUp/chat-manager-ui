  ...
  <a-card :bordered="false">
    <div class="table-page-search-wrapper">
     ...
     <div class="table-operator">
      ...
      <a-dropdown v-action:edit v-if="selectedRowKeys.length > 0" v-has="{perm:'news:batch'}">
        <a-menu slot="overlay">
    ...
    <span slot="action" slot-scope="text, record">
        <template>
        <a-tag color="orange" @click="handleEdit(1,record)" v-has="{perm:'news:edit'}">编辑</a-tag>
        <a-tag
            color="red" @click="handleDelete(record)" v-has="{perm:'news:delete'}">删除</a-tag>
        </template>
    </span>
	</s-table>
...
export default {
  ...
  methods: {
    ...
    handleEdit (__type, record) {
      ...
      this.drawer01.title = __type === 0 ? '新增文章' : '编辑文章'
      this.$nextTick(() => {
