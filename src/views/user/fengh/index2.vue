  ...
  <a-card :bordered="false">
    ...
    <!-- :rowKey="(record) => record.digitId" -->
      ...
      <span slot="identity" slot-scope="identity">
       ...
       <span slot="deviceName" slot-scope="deviceName">
        <template>
          {{ deviceName | isEmpty(true)  }}
        </template>
      </span>


       <span slot="ipAdress" slot-scope="ipAdress">
        <template>
         {{ ipAdress | isEmpty(true)  }}
        </template>
      </span>
...
import { getDeviceForbiddenList,cancelDevice } from "@/api/system/forbiddenManager";
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          dataIndex: "ipAdress",
          width: "180px",
          scopedSlots: { customRender: "ipAdress" },
        },
         {
          title: "封号时间",
          dataIndex: "startTime",
      ...
      loadData: (parameter) => {
        ...
        return getDeviceForbiddenList(Object.assign(parameter, this.queryParam))
          .then((response) => {
            console.log("fenghsb==>", response);
            let res = {
              // hasNext: true,
  ...
  methods: {
    ...
    handleDelete(record) {
      ...
      this.$confirm({
        ...
        onOk() {
          cancelDevice({forbiddenId:record.id })
            .then(response => {
