      ...
      <a-form :form="form">
        ...
        <a-form-item label="推荐模块" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-select  v-decorator="['type']" @change="selectChange" >
            <a-select-option :value="0"> 推荐-首页 </a-select-option>
        ...
        <a-form-item label="推荐时间段一" :labelCol="labelCol" :wrapperCol="wrapperCol" v-if='type==4 || type==0?true:false'>
          <a-time-picker  :default-open-value="moment('00:00', 'HH:mm')" v-decorator="['startTime']"  format="HH:mm" @change="startOnChange"/> 至
        </a-form-item>
        <a-form-item  style="position: absolute; left: 58%; margin-top:-65px;"  :labelCol="labelCol" :wrapperCol="wrapperCol" v-if='type==4|| type==0?true:false'>
         ...
         <a-form-item label="推荐时间段二" :labelCol="labelCol" :wrapperCol="wrapperCol" v-if='(type==4|| type==0)?true:false'>
...
export default {
 ...
 edit(record) {
      ...
      this.$nextTick(()=>{
        setTimeout(() => {
          // if(record.startTime2 == undefined && record.endTime2 == undefined){
        ...
        if(record.type == 4 || record.type == 0 ){
    ...
    handleSubmit() {
      ...
      that.form.validateFields((errors, values) => {
          if (!errors) {
            ...
            var values6= JSON.parse(JSON.stringify(values)); //深克隆
            values6.type = 6;
            var values7= JSON.parse(JSON.stringify(values)); //深克隆
            values7.type = 7;
            ...
            if(values.type==0){
                ...
                anchorRecommendHandle(values6).then(response => {
                  if (response.code === 0) {
                      // that.$message.info(response.message)
                      // that.$emit('searchForm');
                      // that.handleCancel()
                ...
                anchorRecommendHandle(values7).then(response => {
                  if (response.code === 0) {
                      that.$message.info(response.message)
                      that.$emit('searchForm');
                      that.handleCancel()
