...
<script>
import { save, getById, update } from "@/api/system/zbdc/xzgl";
  ...
  methods: {
    add(record) {
      ...
      if (record) {
        ...
        this.setVal(record.id, record.id + random(600, 900), record.level + 1);
      } else {
        this.parentName = "根目录";
        this.setVal(0, this.orgId + random(600, 900), 0);
      }
    },
    setVal(__parentId, __structId, __level) {
      this.$nextTick(() => {
