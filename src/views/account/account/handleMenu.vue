...
<script>
import { queryAssignMenu, saveAssignMenu } from "@/api/system/role";
export default {
  data() {
    return {
      labelCol: {
  ...
  methods: {
    assignMenu(record) {
      ...
      queryAssignMenu({ roleId: record.id }).then(response => {
        ...
        this.checkedKeys = !response.res.attributes
          ? []
          : response.res.attributes.checkerKeys;
      });
    },
    ...
    saveMyAssignMenu() {
      ...
      saveAssignMenu(this.allCkKeys).then(response => {
        if (response.code === 0) {
          ...
          this.$emit("this.$emit('searchForm')");
          this.handleCancel();
        } else {
