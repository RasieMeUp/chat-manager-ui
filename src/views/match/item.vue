    ...
    >
      ...
      <span slot="action" slot-scope="text, record">
        <template>
          <a-tag
            color="teal"
            @click="handleItemUser(record.id)"
            v-has="{ perm: 'ndpx:edit' }"
            >查看投票</a-tag
          >
          <a-tag
...
import formHandle from "./itemhandle";
import formUser from "./itemuser";
import { page, delByPk, batch } from "@/api/matchitem";
...
export default {
  ...
  methods: {
    // getData(id){
    //     page(Object.assign(parameter, this.queryParam)).then(response => {
    //       return response.res
    //     })
    // },
    tableOption() {
    ...
    handleEdit(__type, record) {
      ...
      this.drawer01.title = __type === 0 ? "新增候选者" : "编辑候选者";
      this.$nextTick(() => {
        if (__type === 1) this.$refs.formHandle.edit(record);
        else this.$refs.formHandle.add(this.queryParam.dto.matchid);
      });
    },
    handleOk() {
    ...
    handleItemUser(id) {
      ...
      this.$nextTick(() => {
        this.$refs.formUser.queryParam.dto.matchitemid = id;
      });
    },
  },
};
