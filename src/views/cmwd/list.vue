<template>
  <a-card :bordered="false">
    ...
    <span slot="action" slot-scope="text, record">
        <template>
        <a-tag color="orange" @click="handleEdit(1,record)" v-has="{perm:'word:edit'}">编辑</a-tag>
        <a-tag color="red" @click="handleDelete(record)" v-has="{perm:'word:delete'}">删除</a-tag>
        </template>
    </span>
	</s-table>
...
import { page, delByPk, batch } from '@/api/word'
...
export default {
  ...
  methods: {
    ...
    handleEdit (__type, record) {
      ...
      this.drawer01.title = __type === 0 ? '新增文档' : '编辑文档'
      this.$nextTick(() => {
